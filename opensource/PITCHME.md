### Open Source - 沒有人是局外人
#### (→ Start)
#### [點此預先提問](https://app2.sli.do/event/cdoxqbn8)

+++

### [古永忠 (Yung-Chung Ku)](http://github.com/ycku)
→ [PostgreSQL台灣使用者社群](http://pgsql-tw.github.io)創始人
![pgsql-tw](https://rawgit.com/pgsql-tw/promotional-materials/master/TWPUG-LOGO/TWPUG-LOGO.png)

---

### [25,300,000 active repos in Github (2017)](https://octoverse.github.com/)

---

## Outline
1. 開源**社群**不可分
2. 開源**並非最好**的解決方案
3. 開源是一種**商業生態系**
4. 開源使得**沒有人是局外人**
5. 開源是**最大利益化**的具體運作方法
6. 開源是**公民社會**的實現方式

#### (↓ 先看 Google 台灣總經理簡立峰談開源)

+++

### "[Google 簡立峰演講：AI 科技的產業應用](https://www.inside.com.tw/2017/09/27/ai-industry)"(2017.9.25) 
#### (摘要開源部份內容)

+++

### Google 訓練了 5000 個工程師去學深度學習
- 但是這裡面證實有用的不到 10 個，而且有用還不知道為什麼有用
- Google 也不知道什麼可以做得很好，所以才會訓練大量的工程師去嘗試
- 大量嘗試，累積經驗

+++
### 台灣一定要建立開源（Open source）的文化
- 問題是開源是一個文化，需要從小建立
- 沒有開源文化，再加上台灣是由上而下的領導，讓這個情況變得更糟糕。
- 讓這些最聰明的人去做最基礎而重要的事情

+++
### 透過開源平台進行程式教育是累積成就感最快的方式
- 更認真學習怎麼寫程式
- 更願意在開源社群裡面貢獻
- 不但能力越來越強，而且這些人才也會被看見

---

## 社群

+++

### 開源主題，以社群匯集同好
- 利益也是一種同好，但不能只談錢。
- 本土化社群很重要，建立本地＂人＂的連結。
- [postgresql.jp](https://www.postgresql.jp/) 流量最大的服務是「日文使用手冊」。
- [pietty](https://sites.google.com/view/pietty-project/home)(forked from putty) 作者表示，外國人不瞭解非英語系使用情境。

+++

### 社群 = 市場
- 一起把餅做大，讓更好的人加入共同經營。
- 中立社群能取得更大的市場。

---

## 並非最好

+++

### 開源，不是最好
### 開源，是慢慢變好

+++

### 開源 or 閉源
- 不是二選一 
- 是各種利弊組合的結果 |
- 開源與閉源共存 |

+++

### 開源的缺點
1. 客群設定不容易聚焦。
2. 客製化功能不一定能快速開發。
3. 無法設立獨家標準。
4. 無法進行純軟體販賣商業模式。
5. 導入社群管理後，單方面私有利益可能減損。

---

## 商業生態系

+++

### [開源政府軟件，保加利亞為世界做出榜樣](https://theinitium.com/article/20160707-dailynews-Bulgaria-open-source-software/)
1. 開源為政府開發的客製化軟體。
2. 由稅金開發的軟體，納稅人有權檢查一切。
3. 開源促進系統安全。

+++

### The Linux kernel
1. "Given enough eyeballs, all bugs are shallow" - Linus's Law
2. 最大的貢獻者是「Nobody」。

+++
### Apache Software Foundation
1. "A Patchy" Server?
2. Contributor-Committer-Member
3. "If it didn't happen on-list, it didn't happen."
4. Project Oversight
5. Meritocracy in Action
+++
### OpenStack ([Pike](http://stackalytics.com/?release=pike))
1. 2,400↑ contributors (210 companies)
2. 2,000↑ reviewers (176 companies)
3. 42% commit from "Nobody", Red Hat:23%, VMware:2.5%
4. Top reviewer: 2,888 reviews(1.96%), 409 commits, 671,762 LOCs

+++?image=assets/open-source-commercial-flow.png&size=auto 90%

---

## 沒有人是局外人

+++

### 讓每個人都成為"局內人"
- 重視每一個人的貢獻。
- 以意見領袖為管理者，確保效率最大化。
- 人人都是意見領袖，平行決策。

+++

### 用貢獻取代監督
- 有問題，沒人做，提出 Pull Request (Github)。
- 降低監督比例 = 減少人力資源浪費。
- 建立以「加分」為主的環境。

---

## 最大利益化

+++

### 個人
1. 展示工作過程
2. 公眾信任
3. 回饋的責任感
4. 樂在其中
5. 由你決定你的排程 
6. 絕佳的宣傳
7. 最真實的面試

+++

### 組織
1. 效率
2. 提升開發者的產值
3. 降低管理成本
4. 站在巨人的肩膀上
5. 讓創意進入市場
6. 顧客回饋
7. 減少重覆性的工作
8. 高手吸引力

+++

### 技術
1. 足夠的目光，讓所有問題浮現
2. Free as in speech, not as in beer
3. 上游軟體的改善和回饋
4. 促進多工處理
5. 更加模組化

+++

### 未來
1. 教育下一代
2. 學習動手做
3. 直接民主
4. 現代軟體發展的實踐
5. 開源就是未來

---

## 公民社會
- 去中心化，流動民主
- 化敵為友，選賢與能

+++

### 開始參與
- 零時政府 g0v.tw | http://g0v.tw
- 公共政策網路參與平台 | http://join.gov.tw/

+++?image=http://g0v.tw/imgs/g0v-about.png&size=auto 90%

---

## Open Source
### 坐而言不如起而行

---

## Thanks
### [給我issue](https://github.com/ycku/pitches/issues)
